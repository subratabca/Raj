...
async function NotificationsByType() {
    try {
        ...
        let res = await axios.get(`/client/notification/${id}`);

        if (res.status === 200) {
            let title = res.data.title;
            document.querySelector('.card-body-title').innerText = title;

            if (res.data.food && res.data.food.name && res.data.food.user) {
                if(title == 'Food Publish Notification'){
                    let content = `
                        <p>Dear <strong>${res.data.food.user.firstName}</strong> your food has been published successfully. Food details are given below:</p>
                    ...
                    let content = `
                        <p>There is a new food request from <strong>${res.data.food.order.user.firstName}</strong>.Need to approve food request.Food details are given below:</p>
                        <p>1. <strong>Food Name:</strong> ${res.data.food.name}</p>
                        <p>2. <strong>Food Gradients:</strong> ${res.data.food.gradients}</p>
                    `;
                    document.querySelector('.mg-b-20.mg-sm-b-30').innerHTML = content;
