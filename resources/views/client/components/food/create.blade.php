      ...
      <div class="modal-body pd-20">
        <form id="save-form">
          <div class="row mg-b-25">
            ...
            <div class="col-lg-8">
              <div class="form-group">
                <label class="form-control-label">Gradients: <span class="tx-danger">*</span></label><br>
            ...
            <div class="col-lg-12">
              <div class="form-group">
                <label class="form-control-label">
                  <input type="checkbox" id="accept_tnc">
                  <span><a href="/client/terms-conditions/food_upload" target="_blank">Accept T&C For Food Upload</a></span><span class="tx-danger">*</span>
                </label>
            ...
            <div class="col-lg-4">
              <label class="form-control-label">Upload Food Image:<span class="tx-danger">*</span></label><br>
            ...
            <div class="col-lg-8">
              <label class="form-control-label">Multiple Image:<span class="text-danger">* (You can upload a maximum of 2 images)</span></label><br>
              <label class="custom-file">
...
function multiImageUrl(input) {
  if (input.files.length > 2) {
    errorToast("You can upload a maximum of 2 images.");
    input.value = ""; // Reset the file input
    $('#multiImage').empty(); // Clear the image previews
    return;
  }
...
async function Save() {
  ...
  if (name.length === 0) {
    errorToast("Food name is required!");
  } else if (gradients.length === 0) {
    errorToast("Food gradients are required!");
  } else if (expire_date.length === 0) {
    errorToast("Food expire date is required!");
  } else if (address.length === 0) {
    errorToast("Food collection address is required!");
  } else if (collection_date.length === 0) {
    errorToast("Food collection date is required!");
  } else if (start_collection_time.length === 0) {
    errorToast("Collection start time is required!");
  } else if (end_collection_time.length === 0) {
    errorToast("Collection end time is required!");
  } else if (description.length === 0 || description === '<p><br></p>') { 
    errorToast("Food description is required!");
  } else if (!accept_tnc) {
    ...
    const config = {
      ...
      try {
        let res = await axios.post("/client/create", formData, config);
        if (res.status === 201) {
