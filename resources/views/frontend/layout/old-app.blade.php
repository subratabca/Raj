...
<script>
    document.addEventListener("DOMContentLoaded", async function () {
        try {
            const menuRes = await axios.get('/menu-list');
            if (menuRes.status === 200 && menuRes.data.status === "success" && menuRes.data.data.length > 0) {
                const firstData = menuRes.data.data[0];
                ...
                if (firstData) {
                    ...
                    document.getElementById('descriptionContactPage').innerText = firstData.description || 'No description available';
                ...
                console.error("Error fetching menu data: Status code", menuRes.status);
            }
        } catch (error) {
