...
async function FoodList(page = 1, date = null, searchQuery = null) {
    try {
        ...
        // If searchQuery is present, data is the array of search results
        const foodData = searchQuery ? data : data.data.data.filter(food => 
        ...
        const resultsCount = `Showing ${foodData.length} results`;
        ...
        if (!searchQuery) { // Only show pagination if there's no search query
...
document.querySelector('.filter-today').addEventListener('click', function(event) {
    event.preventDefault();
    FoodList(1, this.getAttribute('data-date'));
});
