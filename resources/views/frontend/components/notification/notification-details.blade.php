        ...
        if (res.status === 200) {
            ...
            if (res.data.food && res.data.user) {
                if(title == 'Approve Food Request Notification'){
                    ...
                    let content = `
                        <p>Dear <strong>${res.data.user.firstName}</strong>, your food request has been has been approved.Please collect your food. Food details are given below:</p>
                    ...
                    let content = `
                        <p>Dear <strong>${res.data.user.firstName}</strong>, your food has been delivered successfully. Food details are given below:</p>
                        <p>1. <strong>Food Name:</strong> ${res.data.food.name}</p>
                        <p>2. <strong>Food Gradients:</strong> ${res.data.food.gradients}</p>
                    `;
                    document.querySelector('.text').innerHTML = content;
