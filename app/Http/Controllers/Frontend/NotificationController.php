...
class NotificationController extends Controller
{
  ...
  public function NotificationList(Request $request)
  {
      try {
          ...
          if ($user) {
              $unreadNotifications = $user->unreadNotifications()->latest()->take(3)->get();
              $unreadCount = $unreadNotifications->count();
              $readNotifications = $user->readNotifications()->latest()->take(3 - $unreadCount)->get();
  ...
  public function MarkAsReadByType(Request $request, $notificationId)
  {
      ...
      // Check if notification data is a string and decode if necessary
      ...
      return view('frontend.pages.dashboard.notification-details-page', compact('type'));
  }
  ...
  public function NotificationDetailsById(Request $request, $notificationId)
  {
      try {
          ...
          // Check if 'data' is a string or already an array
          $notificationData = $notification->data;
          if (is_string($notificationData)) {
              $notificationData = json_decode($notificationData, true);
          }

          // Ensure the 'data' is an array after decoding
          if (!is_array($notificationData)) {
              return response()->json([
                  'status' => 'failed',
          ...
          $food = Food::find($notificationData['food_id']);
